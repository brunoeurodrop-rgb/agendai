...
import { Plus, X, Clock, DollarSign, Pencil } from 'lucide-react'
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Servicos</h1>
          <p className="text-sm text-gray-500 mt-0.5">{services.filter(s => s.active).length} ativos</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {services.length === 0 && <div className="col-span-3 text-center py-16 text-gray-400 text-sm">Nenhum servico cadastrado ainda.</div>}
      </div>
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-semibold text-gray-900">{editing ? 'Editar servico' : 'Novo servico'}</h2>
            ...
            <div className="space-y-4">
              ...
              <div><label className="label">Descricao</label><input className="input" placeholder="Opcional" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="label">Duracao (min)</label><input className="input" type="number" value={form.duration_min} onChange={e => setForm(f => ({ ...f, duration_min: e.target.value }))} /></div>
                <div><label className="label">Preco (R$) *</label><input className="input" type="number" step="0.01" placeholder="0,00" value={form.price} onChange={e => setForm(f => ({ ...f, price: e.target.value }))} /></div>
              </div>
              <div>
