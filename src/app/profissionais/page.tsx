...
export default function ProfissionaisPage() {
  ...
  function openNew() {
    ...
    setEditing(null)
    setForm({ name: '', specialty: '', phone: '', email: '', bio: '', active: true })
    setModal(true)
  }

  function openEdit(p: Professional) {
    setEditing(p)
    setForm({ name: p.name, specialty: p.specialty || '', phone: p.phone || '', email: p.email || '', bio: p.bio || '', active: p.active })
    setModal(true)
  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl">
            ...
            <div className="space-y-4">
              ...
              <div><label className="label">Bio</label><input className="input" value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))} placeholder="Pequena descrição (opcional)" /></div>
            </div>
